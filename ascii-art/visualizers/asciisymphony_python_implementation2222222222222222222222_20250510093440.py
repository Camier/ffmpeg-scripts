...
class ErrorHandler:
    ...
    def check_ffmpeg_capabilities(self):
        """Optimized FFmpeg capability check."""
        capabilities = {}
        try:
            # Check installed
            ...
            capabilities["installed"] = ffmpeg_version.returncode == 0

            # ✅ CORRECTED: Check for libcaca output device support
            ...
            capabilities["libcaca"] = " caca" in format_info.stdout

            # Check for libplacebo (GPU) and hardware accelerations
            filter_info = subprocess.run(
                ["ffmpeg", "-hide_banner", "-filters"],
                capture_output=True,
                text=True,
                timeout=3
            )
            capabilities["libplacebo"] = "libplacebo" in filter_info.stdout

            hwaccel_info = subprocess.run(
                ["ffmpeg", "-hide_banner", "-hwaccels"],
                capture_output=True,
                text=True,
                timeout=2
            )
            capabilities["vulkan"] = "vulkan" in hwaccel_info.stdout
            capabilities["cuda"] = "cuda" in hwaccel_info.stdout

            # Update config for GPU use
            self.config.update({
                "vulkan": 1 if capabilities.get("vulkan") else 0,
                "gpu": 1 if capabilities.get("libplacebo") else 0
            })

            # ⚠️ Optional: warn if libcaca available but unicode not supported
            if capabilities.get("libcaca"):
                # Check if unicode is supported by caca driver
                charset_test = subprocess.run(
                    ["ffmpeg", "-f", "lavfi", "-i", "nullsrc", "-f", "caca", "-list_dither", "charsets", "-"],
                    capture_output=True,
                    text=True
                )
                if "unicode" not in charset_test.stdout:
                    self.logger.warning("FFmpeg caca doesn't support unicode charset")

            return capabilities

        except (subprocess.SubprocessError, TimeoutError, FileNotFoundError) as e:
            self.logger.warning("FFmpeg check failed, assuming minimal capabilities")
...
class TerminalRenderer(Renderer):
    ...
    def render(self, input_stream, output_stream=None):
        ...
        cmd.extend([
            ...
            '-f', 'caca',
            '-color', 'full',
            '-charset', self.config.get('charset', 'ascii'),
