...
[0:a]showspectrum=s=${WIDTH}x${HEIGHT}:mode=combined:slide=scroll:scale=cbrt:color=intensity:gain=5:saturation=10[spectrum];\
[0:a]showwaves=s=${WIDTH}x${HEIGHT}:r=30:mode=p2p:scale=sqrt:colors=yellow:draw=full[wave];\
[0:a]avectorscope=s=${WIDTH}x${HEIGHT}:zoom=1.5:rc=0:gc=1:bc=0:draw=line[vector];\
...
[text]eq=brightness=0.06:saturation=2:gamma=1.1,hue=h=t*2[colored];\
[colored]noise=alls=10:allf=t,format=yuv420p[video]\
" \
-map "[video]" -map 0:a \
