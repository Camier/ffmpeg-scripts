...
# Version: 2.1.1
# Features:
# - GPU-accelerated rendering (libplacebo/Vulkan)
...
LOGDIR="${TMPDIR:-/tmp}/asciisymphony"
mkdir -p "$LOGDIR"
exec 3>&2 2>>"${LOGDIR}/asymphony.log"

# =============================================================================
# CONFIGURATION
# =============================================================================
declare -A PRESETS=(
    [youtube]="--fps 30 --quality vp9 --colors vibrant"
    [terminal]="--charset extended --dither ordered2 --effects retro"
    [studio]="--resolution 4k --hdr --renderer vulkan"
)

DEFAULTS=(
    "--mode adaptive" "--colors thermal" "--quality balanced"
    "--fps $(($(nproc) > 4 ? 30 : 15))" "--threads $(( $(nproc) / 2 ))"
)

# =============================================================================
# PRESET MANAGEMENT
# =============================================================================
PRESET_DIR="${HOME}/.asciisymphony/presets"
...
AUDIO_DEVICES=()
LIVE_SETTINGS=(
    "--buffer-size 1024" "--latency low" "--monitor 1"
...
log() {
    echo "[INFO] $*" >&3
    echo "[$(date +'%Y-%m-%d %H:%M:%S')] [INFO] $*" >> "${LOGDIR}/asymphony.log"
}
...
init_engine() {
    echo "DEBUG: Entering init_engine"
    local -gA config=(
        [fps]=30                [width]=1280           [height]=720
        [quality]="balanced"    [encoder]="h264"       [vulkan]=0
        [hdr]=0                 [gpu]=0                [latency]="normal"
        [mode]="spectrosynth"   [charset]="unicode"    [dither]="fstein"
        [hue]=1.5               [saturation]=1.2       [bitrate]="auto"
        [live_input]=0          [sample_rate]=44100    [channels]=2
        [buffer_size]=1024
    )
    echo "DEBUG: config array initialized"
    
    echo "DEBUG: Before detect_hardware"
    detect_hardware
    echo "DEBUG: After detect_hardware, vulkan=${config[vulkan]}, gpu=${config[gpu]}"
    
    # Don't even try to load presets, as it might be causing issues
    # if [[ -n "${PRESETS[$1]}" ]]; then
    #     echo "DEBUG: Loading preset for \"$1\""
    #     load_preset "${PRESETS[$1]}"
    #     shift
    # fi
...
detect_hardware() {
    [[ $(ffmpeg -hwaccels 2>&1) =~ vulkan ]] && config[vulkan]=1
    [[ $(ffmpeg -v quiet -filters | grep libplacebo) ]] && config[gpu]=1
    return 0
}

parse_args() {
    echo "DEBUG: parse_args called with args: $*"
    for arg in "$@"; do
        echo "DEBUG: processing arg: $arg"
        case "$arg" in
            --mode=*)
                config[mode]="${arg#*=}"
                echo "DEBUG: set mode=${config[mode]}"
                ;;
            --fps=*)
                config[fps]="${arg#*=}"
                echo "DEBUG: set fps=${config[fps]}"
                ;;
            --quality=*)
                config[quality]="${arg#*=}"
                echo "DEBUG: set quality=${config[quality]}"
                ;;
            --colors=*)
                config[colors]="${arg#*=}"
                echo "DEBUG: set colors=${config[colors]}"
                ;;
            --charset=*)
                config[charset]="${arg#*=}"
                echo "DEBUG: set charset=${config[charset]}"
                ;;
            --dither=*)
                config[dither]="${arg#*=}"
                echo "DEBUG: set dither=${config[dither]}"
                ;;
            --hue=*)
                config[hue]="${arg#*=}"
                echo "DEBUG: set hue=${config[hue]}"
                ;;
            --saturation=*)
                config[saturation]="${arg#*=}"
                echo "DEBUG: set saturation=${config[saturation]}"
                ;;
            *)
                echo "DEBUG: unrecognized arg: $arg"
                ;;
...
create_visualization() {
    echo "DEBUG: Starting visualization processing"
    
    local filter_chain=$(build_filter_chain)
    filter_chain="${filter_chain%,}"  # Remove trailing comma
    
    echo "DEBUG: Using filter chain: $filter_chain"
    
    # Add GPU acceleration if detected - using more compatible options
    local hwaccel_args=()
    if [[ "${config[vulkan]}" -eq 1 ]]; then
        # Just use general hwaccel without vulkan-specific options
        hwaccel_args=(-hwaccel auto)
    fi

    ffmpeg -y "${hwaccel_args[@]}" -i "$input" \
        -filter_complex "$filter_chain" -an \
        -c:v libx264 -preset medium -crf 23 \
        "$output" 2>&1

    local exit_code=$?
    if [[ $exit_code -eq 0 ]]; then
        echo "DEBUG: Successfully created output file: $output"
    else
        echo "DEBUG: FFmpeg command failed with exit code $exit_code"
    fi
...
build_filter_chain() {
    local chain=()
    # Only use hwupload for video inputs, not for audio visualizations, 
    # as hwupload is for video processing
    # [[ "${config[gpu]}" -eq 1 ]] && chain+=("hwupload")
    
    case "${config[mode]}" in
        "spectrosynth")
            chain+=("asplit=3[main][spec][wave]"
                "[main]showfreqs=scale=log:win_size=2048"
                "[spec]showspectrum=mode=combined:slide=scroll"
                "[wave]showwaves=mode=p2p:split_channels=1"
                "vstack=inputs=3")
            ;;
        "vortex")
        ...
        "waves")
            ...
            ;;
        "spectrum")
            chain+=("showspectrum=s=${config[width]}x${config[height]}:mode=combined")
            ;;
        "cqt")
        ...
        "typography")
            ...
            # Create temporary lyrics file with audio-reactive words
...
EOF
            chain+=("[0:a]asplit=2[a1][a2],"
                   ...
                   "drawtext=textfile=/tmp/lyrics.txt:reload=1:fontsize='w/20*sin(t)+w/10':"
        ...
        "fractal")
            ...
            ;;
        "ritualfusion")
            # Cross-aesthetic: spectrum + waveform + fractal + dynamic text + kaleidoscope
            cat > /tmp/ritual_words.txt <<EOF
RITUAL
FUSION
SPECTRUM
WAVE
...
EOF
            chain+=(
                ...
                "[a1]showspectrum=s=${config[width]}x$((config[height]/3)):mode=combined:color=rainbow[spec],"
                "[a2]showwaves=s=${config[width]}x$((config[height]/3)):mode=cline:colors=0xffffff[wave],"
                "[a3]showcqt=s=${config[width]}x$((config[height]/3)):count=8:gamma=4[cqt],"
                "[spec][wave][cqt]vstack=inputs=3[stacked],"
                "[stacked]kaleidoscope=pattern=2:angle=PI/6[kal],"
                "[kal]drawtext=textfile=/tmp/ritual_words.txt:reload=1:fontsize='h/12+20*sin(t)':x='w/2-w/8*sin(t/2)':y='h/2+h/4*cos(t/3)':fontcolor=00ffff@0.7,format=rgb24"
            )
            ;;
        *)
...
load_preset() {
    ...
    if [[ -z "$preset_name" ]]; then
        warning "No preset name provided"
        echo "Usage: $0 --load-preset PRESET_NAME"
        return 1
    fi
    ...
    # Load preset
    log "Loading preset: $(basename "$preset_name" .preset)"
    
    # Source the preset file to load variables into config
...
disable_feature() {
    log "Disabling feature: $1"
    # Implementation to disable specific features based on error
    if [[ "$1" == *"Hardware"* ]]; then
        config[gpu]=0
        config[vulkan]=0
    fi
...
main() {
    # Restore the trap but make sure it doesn't cause issues
    trap 'emergency_shutdown 2>/dev/null || true' EXIT INT TERM
    ...
    initialize_preset_system
    echo "DEBUG: Preset system initialized"
    ...
    init_engine "$@"
    echo "DEBUG: init_engine complete"
    
    echo "DEBUG: About to validate input for $input"
    validate_input "$input" || echo "DEBUG: validate_input failed"
    echo "DEBUG: Input validated"
    
    echo "DEBUG: Checking dependencies"
    check_dependencies || echo "DEBUG: check_dependencies failed"
    echo "DEBUG: Dependencies checked"
    
    echo "DEBUG: Allocating resources"
    allocate_resources || echo "DEBUG: allocate_resources failed"
    echo "DEBUG: Resources allocated"
    
    echo "DEBUG: Running create_visualization"
    create_visualization
    echo "DEBUG: Visualization complete"
...
validate_input() {
    log "Running validate_input on: $1"
    echo "DEBUG: Validating input: $1"
    # Skip validation for live input
    [[ "${config[live_input]}" -eq 1 ]] && { echo "DEBUG: Live input, skipping validation"; return 0; }
    
    if [[ ! -f "$1" ]]; then
        echo "DEBUG: Input file not found: $1"
        return 1 # don't exit, just return error for debugging
    fi
    
    # Check if file is readable by FFmpeg - use explicit path
    echo "DEBUG: Checking if FFmpeg can read: $1"
    if ! ffprobe -v error -i "$1" -show_entries format=duration -of default=noprint_wrappers=1:nokey=1; then
        echo "DEBUG: FFmpeg cannot read the input file: $1"
        return 1 # don't exit, just return error for debugging
    fi
    
    echo "DEBUG: Input validated successfully"
    return 0
}
...
show_help() {
    ...
    echo "  --list-modes       List all available visualization modes"
    echo "  --screenshot [time] [input_file] [output_image]"
    echo "                     Output a single frame as an image (default time: 00:00:01)"
    ...
    echo "  ritualfusion - Cross-aesthetic: spectrum, waveform, fractal, text, recursion"
    echo ""
...
list_modes() {
    ...
    echo "  ritualfusion  - Cross-aesthetic: spectrum, waveform, fractal, text, recursion"
    echo ""
    echo "Use: $0 --mode=MODE ..."
}

show_version() {
    echo "AsciiSymphony Pro version 2.1.1"
}

take_screenshot() {
    local seek_time="${1:-00:00:01}"
    local input_file="${2:-input.mp3}"
    local output_image="${3:-screenshot.png}"
    ...
    # Use current config or defaults for visualization mode
    local filter_graph
    filter_graph=$(build_filter_chain)

    # Output a single frame as PNG at the specified time
    ffmpeg -v warning -y -ss "$seek_time" -i "$input_file" \
        -lavfi "$filter_graph" -frames:v 1 "$output_image"

    if [[ $? -eq 0 && -f "$output_image" ]]; then
        echo "Screenshot saved to: $output_image"
        return 0
    else
        echo "[ERROR] Failed to create screenshot" >&2
        return 1
    fi
}
